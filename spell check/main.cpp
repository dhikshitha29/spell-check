...
char* metaphone(char word[])
{
    ...
    if(f==1)                                //if f=1 drop the first letter
    {
    ...
    if(word[n-1]=='b')                      //check if the word ends with MB
    {
        if(word[n-2]=='m')
        {
            f=1;
    ...
    if(f==1)
    {
        for(i=0;i<n-1;i++)
        {
            wrd[i]=word[i];             ///drop the last letter 'b'
        }
        wrd[i]='\0';
        strcpy(word,wrd);
    }
    ...
    for(i=0;i<n;i++)
    {
        ...
        else
        {
            switch(word[i+1])
            {
                case 'h':if(word[i-1]=='s')             ///IF SCH ENCOUNTERED ADD K TO WRD[J] AFTER S - (SCH~SKH)
                        {
                            wrd[j]='k';
                        }
                        else
                        {
                            wrd[j]='x';                 ///X IS ADDED TO WORD[J] - *CH~*XH
                        }
                        break;
    ...
    for(i=0;i<n;i++)
    {
        ...
        else
        {
            if(i<n-2)
            {
                if(word[i+1]=='g')
                {
                    if((word[i+2]=='e')||(word[i+2]=='i')||(word[i+2]=='y'))  ///DGE~JGE,DGY~JGY,DGI~JGI
                    {
                        f=1;
                    }
                }
    ...
    ///Drop 'G' if followed by 'N' or 'NED' is at the end.
    ...
    for(i=0;i<n;i++)
    {
        ...
        {
          if(word[i+1]=='h'&&i!=n-2)                ///To check the whether h appears after g and h not in end of word
          {
              for(k=0;k<5;k++)
              {
                  if(word[i+2]==vowels[k])          /// to check if GH is before a vowel -> GHE~*HE
                  {
                      g=1;
                  }
              }
          ...
          {
              if(word[i+1]=='n'&&i==n-2)            ///if [i+1]=n and is in the end then drop G
              {
                  f=1;
              }
          }
    ...
    ///STEP 7:'G' transforms to 'J' if before 'I', 'E', or 'Y'.Otherwise, 'G' transforms to 'K'.
    ...
    for(i=0;i<n;i++)
    {
        ...
        {
            switch(word[i+1])           ///GI,GE,GY ~ JI,JE,JY respectively
            {
                case 'i':
...
//to read the contents from file
    ...
    cout<<"\t\t\t\t\t**************************************\t\t\t"<<endl;
    cout<<"\t\t\t\t\t**\t    Thank you!!!\t    **"<<endl;
    cout<<"\t\t\t\t\t**\t  Spell Checker By:\t    **"<<endl;
    cout<<"\t\t\t\t\t**\t  19PD09-DHIKSHITHA A\t    **"<<endl;
    cout<<"\t\t\t\t\t**\t  19PD38-SWATHI PRATHAA P   **"<<endl;
    cout<<"\t\t\t\t\t**************************************\t\t\t"<<endl;
    cout<<"\t\t\t\t\t**************************************\t\t\t";
}
